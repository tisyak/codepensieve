...
public class MasterDataDAOImpl implements MasterDataDAO {
	...
	public MasterData get(Class subClass,String id)
		...
		if (subClass.getName().contains("ImagePasswordDetail")) {
			masterData = (MasterData) getCurrentSession().get(
					subClass, Integer.parseInt(id));
		} else {
			masterData = (MasterData) getCurrentSession().get(
					subClass, id);
		}
		if (masterData == null) {
			...
			throw new EmptyResultDataAccessException("masterData paraname ["
					+ id + "] not found", 1);
		} else {
	...
	@Override
	public void update(Class subClass,MasterData masterData)
			throws EmptyResultDataAccessException {
		...
		if (masterDataToUpdate != null) {
			...
			throw new EmptyResultDataAccessException("MasterData  - for "+ subClass.getName() +"["
					+ masterData.getUniqueId() + "] not found", 1);
		}
	}

	@Override
	public void delete(Class subClass,String id)
			throws EmptyResultDataAccessException {
	...
	public List<MasterData> getAll(String modelName) {
		return getCurrentSession().createQuery("from "+modelName).list();
	}
