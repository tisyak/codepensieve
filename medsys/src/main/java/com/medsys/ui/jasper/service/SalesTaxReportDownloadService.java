...
import com.medsys.orders.bd.SalesTaxJasperDatasourceService;
...
public class SalesTaxReportDownloadService {
	...
	@Autowired
	private SalesTaxJasperDatasourceService datasource;
	...
	public void download(String type, String token, Date fromDate,Date toDate, HttpServletResponse response) {
		...
		try {
			...
			params.put("Title", "Sales Tax Report FROM [" + simpleDateFormat.format(fromDate) + "] TO [" + simpleDateFormat.format(toDate) + "]");
			...
			JasperPrint jp = JasperFillManager.fillReport(jr, params, datasource.getSalesTaxData(fromDate,toDate));
